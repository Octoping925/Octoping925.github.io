...
import * as ReplayTypes from "./constants.js";
...
import { heroAttribute } from "./attr.js";

// uncomment for debug
// log.level = 'trace';

// 2.55.2.87306
export const MAX_SUPPORTED_BUILD = 87774;

const BSTEP_FRAME_THRESHOLD = 8;

export const ReplayDataType = {
  game: "gameevents",
...
const ReplayToProtocolType = {
  ...
  attributeevents: ATTRIBUTES_EVENTS,
  header: HEADER,
  details: DETAILS,
  initdata: INITDATA,
...
export function parse(file, requestedData, opts) {
  const replay = {};

  // execute sync
  for (const i in requestedData) {
    log.debug("Retrieving " + requestedData[i]);
    replay[requestedData[i]] = get(
  ...
  if (opts) {
    if ("saveToFile" in opts) {
      writeFile(opts.saveToFile, JSON.stringify(replay, null, 2), (err) => {
        if (err) throw err;
  ...
  if (replay[ReplayDataType.lobby]) {
    replay.tags = getBattletags(
      replay[ReplayDataType.lobby],
      replay.details.m_playerList,
    );
  }
...
export function getHeader(file) {
  try {
    ...
    // players
    match.playerIDs = details.m_playerList.map((pdata) => {
      return (
...
export function getBattletags(buffer, playerList) {
  if (!buffer) return [];

  const btagRegExp = XRegExp("(\\p{L}|\\d){3,24}#\\d{4,10}[zØ]?", "g");
  const matches = buffer.toString().match(btagRegExp);
...
export function processReplay(file, opts = {}) {
  // options
      ...
      if (event._eventid === ReplayTypes.TrackerEvent.Stat) {
        if (event.m_eventName === ReplayTypes.StatEventType.PlayerInit) {
          ...
          playerIDMap[event.m_intData[0].m_value] =
            event.m_stringData[1]?.m_value ?? "0-\u0000\u0000\u0000\u0000-0-0";
          ...
          players[
            event.m_stringData[1]?.m_value ?? "0-\u0000\u0000\u0000\u0000-0-0"
          ].hero = opts.useAttributeName ? attrName : heroAttribute[attrName];
          ...
          log.trace(
            "Player " + event.m_stringData[1]?.m_value ??
              "0-\u0000\u0000\u0000\u0000-0-0" +
                " has tracker ID " +
      ...
      if (event._eventid === ReplayTypes.TrackerEvent.Score) {
        ...
        if (match.map === ReplayTypes.MapType.HauntedMines) {
          ...
          if (dragon && dragon.tag === tag && dragon.rtag === rtag) {
            ...
            match.objective[dragon.team].events[lastIdx].duration =
              loopsToSeconds(
                match.objective[dragon.team].events[lastIdx].loopDuration,
              );
...
function braxisWaveStrength(units, build) {
  ...
  if (build < 66488) {
    ...
    score = Math.max(
      score,
      0.3 * types[ReplayTypes.BraxisUnitType.ZergGuardian],
    );
...
function analyzeUptime(match, players) {
  ...
  const team0Uptime = analyzeTeamPlayerUptime(0, players);
  const team1Uptime = analyzeTeamPlayerUptime(1, players);
...
function analyzePlayerHeroUptime(player) {
  ...
  const intervals = [];
  for (const unitId in player.units) {
...
export function getFirstObjectiveTeam(match) {
  try {
    if (
...
export function getFirstFortTeam(match) {
  const t0Fort = match.teams[0].stats.structures.Fort;
  const t1Fort = match.teams[1].stats.structures.Fort;
...
export function getFirstKeepTeam(match) {
  const t0Keep = match.teams[0].stats.structures.Keep;
  const t1Keep = match.teams[1].stats.structures.Keep;
...
export function winFileTimeToDate(filetime) {
  return new Date(filetime / 10000 - 11644473600000);
}

export function loopsToSeconds(loops) {
  // apparently hots does 16 updates per second
  return loops / 16;
}

export const VERSION = PARSER_VERSION;
